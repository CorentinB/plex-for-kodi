...
<focusedlayout width="240">
    <control type="group">
        <posx>60</posx>
        <posy>{{ vscale(10) }}</posy>
        <animation effect="zoom" start="100" end="105" time="110" center="100,{{ vscale(100) }}" reversible="true" condition="Control.HasFocus({{ role_focus_id }})">Conditional</animation>
        <control type="image">
            <visible>Control.HasFocus({{ role_focus_id }})</visible>
            <posx>-5</posx>
            <posy>{{ vscale(-5) }}</posy>
            <width>210</width>
